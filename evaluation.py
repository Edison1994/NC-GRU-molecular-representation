...
def parallel_eval_qsar(model, step, hparams):
    ...
    embedding_array, inp_seq, inp_len = sequence2embedding(model, hparams, seq_list)
    process = mp.Process(
        target=eval_qsar,
...
def eval_reconstruct(model, step, hparams, data_eval='chembl_val'):
    ...
    if data_eval == 'igc50_val':
        header = ["step"] + list(model.model.measures_to_log.keys())
        fields = [step]
        if step == 0:
            with open(os.path.join(hparams.save_dir, "eval_igc50.csv"), 'w') as f:
                writer = csv.writer(f)
                writer.writerow(header)
        measures = []
        while True:
            try:
                ...
                break
        fields.extend(np.mean(measures, axis=0).tolist())
        with open(os.path.join(hparams.save_dir, "eval_igc50.csv"), 'a') as f:
            writer = csv.writer(f)
            writer.writerow(fields)
    if data_eval == 'lc50dm_val':
        header = ["step"] + list(model.model.measures_to_log.keys())
        fields = [step]
        if step == 0:
            with open(os.path.join(hparams.save_dir, "eval_lc50dm.csv"), 'w') as f:
                writer = csv.writer(f)
                writer.writerow(header)
        measures = []
        while True:
            try:
                ...
                break
        fields.extend(np.mean(measures, axis=0).tolist())
        with open(os.path.join(hparams.save_dir, "eval_lc50dm.csv"), 'a') as f:
            writer = csv.writer(f)
            writer.writerow(fields)

    if data_eval == 'lc50_val':
        header = ["step"] + list(model.model.measures_to_log.keys())
        fields = [step]
        if step == 0:
            with open(os.path.join(hparams.save_dir, "eval_lc50.csv"), 'w') as f:
                writer = csv.writer(f)
                writer.writerow(header)
        measures = []
        while True:
            try:
                ...
                break
        fields.extend(np.mean(measures, axis=0).tolist())
        with open(os.path.join(hparams.save_dir, "eval_lc50.csv"), 'a') as f:
            writer = csv.writer(f)
            writer.writerow(fields)

    if data_eval == 'ld50_val':
        header = ["step"] + list(model.model.measures_to_log.keys())
        fields = [step]
        if step == 0:
            with open(os.path.join(hparams.save_dir, "eval_ld50.csv"), 'w') as f:
                writer = csv.writer(f)
                writer.writerow(header)
        measures = []
        while True:
            try:
                ...
                break
        fields.extend(np.mean(measures, axis=0).tolist())
        with open(os.path.join(hparams.save_dir, "eval_ld50.csv"), 'a') as f:
            writer = csv.writer(f)
            writer.writerow(fields)
