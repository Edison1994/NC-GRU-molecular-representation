...
def add_arguments(parser):
    ...
    parser.add_argument('--unknown_num', default=512, type=int)
...
def main(unused_argv):
    ...
    if FLAGS.preprocess:
        df = df.join(pd.DataFrame(descriptors,
                                  ...
                                  columns=["cddd_" + str(i+1) for i in range(FLAGS.unknown_num)])) # used to be FLAGS.unknown_num=512
    else:
        df = df.join(pd.DataFrame(descriptors,
                                  index=df.index,
                                  columns=["cddd_" + str(i+1) for i in range(FLAGS.unknown_num)]))
