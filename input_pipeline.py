...
class InputPipeline():
    ...
    def __init__(self, mode, hparams, data_chosen='chembl_val'):
        """Constructor for base input pipeline class.

        Args:
            mode: The mode the model is supposed to run (e.g. Train).
            hparams: Hyperparameters defined in file or flags.
            data_chosen: The data chosen for evaluation (e.g. chembl_val, igc50, lc50dm...)
        Returns:
            None
        """
        self.mode = mode
        self.data_chosen = data_chosen
        ...
        if self.mode == "TRAIN":
            ...
            if self.data_chosen == 'chembl_val':
                self.input_sequence_key = "canonical_smiles"
                self.file = hparams.val_file
            elif self.data_chosen == 'igc50_val':
                self.input_sequence_key = "canonical_smiles"
                self.file = hparams.val_igc50
            elif self.data_chosen == 'lc50dm_val':
                self.input_sequence_key = "canonical_smiles"
                self.file = hparams.val_lc50dm
            elif self.data_chosen == 'lc50_val':
                self.input_sequence_key = "canonical_smiles"
                self.file = hparams.val_lc50
            elif self.data_chosen == 'ld50_val':
                self.input_sequence_key = "canonical_smiles"
                self.file = hparams.val_ld50
...
class InputPipelineWithFeatures(InputPipeline):
    ...
    def __init__(self, mode, hparams, data_chosen='chembl_val'):
        ...
        Returns:
            None
        """
        super().__init__(mode, hparams, data_chosen)
